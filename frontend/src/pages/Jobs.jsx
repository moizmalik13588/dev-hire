...
const Jobs = () => {
  ...
  const JobDetail = ({ job }) => (
    <div className="bg-white dark:bg-gray-800 h-full p-5 overflow-y-auto rounded-lg">
      ...
      <p className="text-blue-600 dark:text-blue-400 font-medium mb-3">{job.company?.name}</p>
      ...
      <div className="mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {job.requiredSkills.map((skill) => (
            <span key={skill} className="bg-blue-50 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-700 px-3 py-1 rounded-full text-sm font-medium">
              {skill}
            </span>
          ))}
        </div>
    ...
    <div onClick={onClick}
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-semibold text-blue-700 dark:text-blue-400 truncate">{job.title}</h3>
          <p className="text-sm text-gray-700 dark:text-gray-300">{job.company?.name}</p>
      ...
      <div className="flex flex-wrap gap-1 mt-2">
        {job.requiredSkills.slice(0, 3).map((skill) => (
          <span key={skill} className="bg-blue-50 dark:bg-blue-900/50 text-blue-600 dark:text-blue-300 text-xs px-2 py-0.5 rounded-full border border-blue-100 dark:border-blue-800">
            {skill}
          </span>
        ))}
      </div>
      <div className="flex items-center justify-between mt-2">
        ...
        <span className="text-xs text-gray-400 dark:text-gray-500 flex items-center gap-1">
          <Users size={11} />{job._count?.applications || 0} applied
        </span>
      </div>
    </div>
  )
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-lg">
            <div className="flex items-center flex-1 px-4 border-b sm:border-b-0 sm:border-r border-gray-200 dark:border-gray-600">
              ...
              <input type="text" placeholder="Job title or keywords" value={search}
      ...
      <div className="max-w-5xl mx-auto px-4 pb-12">
        {loading ? (
          ...
          <div className="text-center py-20 text-gray-400">
            <Briefcase size={48} className="mx-auto mb-4" /><p>No jobs found</p>
          </div>
        ) : (
          <>
            {/* Mobile */}
            <div className="md:hidden">
              {showDetail && selectedJob ? (
                ...
                <div className="space-y-2">
                  {jobs.map((job) => (
                    <JobCard key={job.id} job={job} selected={false}
