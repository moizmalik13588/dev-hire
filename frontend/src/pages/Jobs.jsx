...
import { MapPin, DollarSign, Search, Briefcase, Clock, Users, ArrowLeft } from 'lucide-react'
...
const Jobs = () => {
  ...
  const JobDetail = ({ job }) => (
    <div className="bg-white h-full p-5 overflow-y-auto">
      ...
      <p className="text-blue-600 font-medium mb-3">{job.company?.name}</p>

      <div className="flex flex-wrap gap-3 text-sm text-gray-600 mb-4">
        {job.location && <span className="flex items-center gap-1"><MapPin size={14} />{job.location}</span>}
        {job.salaryMin && <span className="flex items-center gap-1"><DollarSign size={14} />PKR {job.salaryMin.toLocaleString()} - {job.salaryMax.toLocaleString()}</span>}
      ...
      {!user && (
        <button onClick={() => navigate('/login')} className="w-full bg-blue-600 text-white py-3 rounded-lg hover:bg-blue-700 font-semibold text-lg mb-5">
          Sign in to Apply
        </button>
      )}
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-blue-700 py-8 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-0 bg-white rounded-lg overflow-hidden shadow-lg">
            <div className="flex items-center flex-1 px-4 border-b sm:border-b-0 sm:border-r border-gray-200">
              ...
              <input
                ...
                className="w-full py-3 focus:outline-none text-gray-700 text-sm"
              />
            </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-3">
        <p className="text-gray-500 text-sm">{loading ? 'Searching...' : `${jobs.length} jobs found`}</p>
      </div>
        ...
        {loading ? (
          ...
          <>
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-semibold text-blue-700 truncate">{job.title}</h3>
                          <p className="text-sm text-gray-600">{job.company?.name}</p>
                          <div className="flex flex-wrap gap-2 mt-1 text-xs text-gray-500">
                            {job.location && <span className="flex items-center gap-1"><MapPin size={10} />{job.location}</span>}
                            {job.salaryMin && <span className="flex items-center gap-1"><DollarSign size={10} />{(job.salaryMin/1000).toFixed(0)}k-{(job.salaryMax/1000).toFixed(0)}k</span>}
                          </div>
                        </div>
                      </div>
                      <div className="flex flex-wrap gap-1 mt-2">
                        {job.requiredSkills.slice(0, 3).map((skill) => (
                          <span key={skill} className="bg-blue-50 text-blue-600 text-xs px-2 py-0.5 rounded-full">{skill}</span>
                        ))}
                      </div>
                      <div className="flex justify-between mt-2 text-xs text-gray-400">
                        <span className="flex items-center gap-1"><Clock size={10} />{timeAgo(job.createdAt)}</span>
                        <span className="flex items-center gap-1"><Users size={10} />{job._count?.applications || 0} applied</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="hidden md:flex gap-6">
                ...
                {jobs.map((job) => (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <span className="text-xs text-gray-400 flex items-center gap-1"><Users size={11} />{job._count?.applications || 0} applied</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {/* Right Detail */}
              {selectedJob && (
                <div className="flex-1 bg-white rounded-lg border border-gray-200 sticky top-20 max-h-[85vh] overflow-y-auto">
                  <JobDetail job={selectedJob} />
                </div>
              )}
            </div>
